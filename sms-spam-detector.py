...
def convert_features(message):
    words_in_message = message.lower().split()
    features = []
    for word in all_words:
        features.append(1 if word in words_in_message else 0)
    return features

X = [convert_features(message) for label, message in messages]
y = [1 if label == "spam" else 0 for label, message in messages]

model = LogisticRegression()
model.fit(X, y)

def predict(message):
    features = convert_features(message)
